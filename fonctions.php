...
function qrcodeOuvert ($partie, $qrcode, $date) {
	...
	if ($row = $req->fetch())
		...
		return true;
	if($delai < (20))		//si le temps écoulé entre maintenant et le dernier flash est inférieur à 20 secondes, alors le QRcode est fermé
		return false;
	else 
		return true;
}
...
function getIdEquipeParZone ($id_partie) {
	...
	while ($row = $req->fetch()) {
		$list[] = array(
			'zone' => strval($row['id_zone']),
			'equipe' => strval(getMeilleureEquipeZone($id_partie, $row['id_zone']))
		);
	}
	return $list;
}
